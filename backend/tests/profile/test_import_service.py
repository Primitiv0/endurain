...
class TestImportServiceGears:
    async def test_collect_and_import_gears(self) -> None:
        ...
        with patch(
            "profile.import_service.gear_crud.create_gear",
    ...
    async def test_collect_and_import_gear_components(self) -> None:
        ...
        with patch("profile.import_service.gear_components_crud.create_gear_component"):
...
class TestImportServiceUserData:
    ...
    async def test_collect_and_import_user_identity_providers_empty(self) -> None:
        mock_db = MagicMock(spec=Session)
        mock_ws = MagicMock()
        service = profile_import_service.ImportService(
            user_id=1,
        ...
        await service.collect_and_import_user_identity_providers([])
        assert service.counts.get("user_identity_providers", 0) == 0

    async def test_collect_and_import_user_identity_providers_no_idps(self) -> None:
        mock_db = MagicMock(spec=Session)
        mock_ws = MagicMock()
        service = profile_import_service.ImportService(
            user_id=1,
        ...
        with patch(
            "profile.import_service.identity_providers_crud.get_all_identity_providers",
            return_value=[],
        ):
            await service.collect_and_import_user_identity_providers([{"idp_id": 1}])
        assert service.counts.get("user_identity_providers", 0) == 0

    async def test_collect_and_import_user_identity_providers_idp_not_found(self) -> None:
        mock_db = MagicMock(spec=Session)
        mock_ws = MagicMock()
        service = profile_import_service.ImportService(
            user_id=1,
        ...
        with (
            ...
            patch(
                "profile.import_service.identity_providers_crud.get_identity_provider",
                return_value=None,
            ),
        ):
            await service.collect_and_import_user_identity_providers([{"idp_id": 999}])
        assert service.counts.get("user_identity_providers", 0) == 0
    ...
    async def test_collect_and_import_user_data(self) -> None:
        ...
        with (
            patch(
                "profile.import_service.users_crud.edit_profile_user",
            ...
            patch.object(service, "collect_and_import_user_identity_providers", new_callable=AsyncMock),
    ...
    async def test_collect_and_import_user_integrations(self) -> None:
        ...
        with patch(
            "profile.import_service.user_integrations_crud.edit_user_integrations",
        ):
    ...
    async def test_collect_and_import_user_goals(self) -> None:
        ...
        with patch("profile.import_service.user_goals_crud.create_user_goal"):
            await service.collect_and_import_user_goals(
    ...
    async def test_collect_and_import_user_privacy_settings(self) -> None:
        ...
        with patch(
            "profile.import_service.users_privacy_settings_crud.edit_user_privacy_settings",
        ):
    ...
    async def test_collect_and_import_user_identity_providers(self) -> None:
        mock_db = MagicMock(spec=Session)
        mock_ws = MagicMock()
        service = profile_import_service.ImportService(
            user_id=1,
        ...
        with (
            ...
            patch("profile.import_service.user_identity_providers_crud.create_user_identity_provider"),
        ):
            await service.collect_and_import_user_identity_providers([{"idp_id": 1, "idp_subject": "user123"}])

        assert service.counts["user_identity_providers"] == 1
...
class TestImportServiceHealth:
    async def test_collect_and_import_health_weight(self) -> None:
        ...
        with (
            patch("profile.import_service.health_weight_crud.create_health_weight"),
            patch(
                "profile.import_service.health_targets_crud.get_health_targets_by_user_id",
                return_value=mock_health_target,
            ),
            patch("profile.import_service.health_targets_crud.edit_health_target"),
        ):
    ...
    async def test_collect_and_import_health_weight_no_targets(self) -> None:
        ...
        with (
            ...
            patch("profile.import_service.health_targets_crud.create_health_targets"),
        ):
            await service.collect_and_import_health_weight(
                [{"weight": 75.5}],
                [],
...
class TestImportServiceNotifications:
    ...
    async def test_collect_and_import_notifications(self) -> None:
        ...
        with patch("profile.import_service.notifications_crud.create_notification"):
...
class TestImportServiceActivityComponents:
    ...
    async def test_collect_and_import_activity_components_with_laps(self) -> None:
        ...
        with patch("profile.import_service.activity_laps_crud.create_activity_laps"):
            await service.collect_and_import_activity_components(
    ...
    async def test_collect_and_import_activity_components_with_sets(self) -> None:
        ...
        with patch("profile.import_service.activity_sets_crud.create_activity_sets"):
            await service.collect_and_import_activity_components(
                [],
    ...
    async def test_collect_and_import_activity_components_with_streams(self) -> None:
        ...
        with patch("profile.import_service.activity_streams_crud.create_activity_streams"):
            await service.collect_and_import_activity_components(
                [],
                [],
    ...
    async def test_collect_and_import_activity_components_with_workout_steps(self) -> None:
        ...
        with patch("profile.import_service.activity_workout_steps_crud.create_activity_workout_steps"):
            await service.collect_and_import_activity_components(
                [],
                [],
                [],
    ...
    async def test_collect_and_import_activity_components_with_exercise_titles(self) -> None:
        ...
        with patch("profile.import_service.activity_exercise_titles_crud.create_activity_exercise_titles"):
            await service.collect_and_import_activity_components(
                [],
                [],
                [],
...
class TestImportServiceActivityComponentsMedia:
    async def test_collect_and_import_activity_components_with_media_valid_path(self) -> None:
        ...
        with (
            patch("profile.import_service.file_uploads.resolve_storage_path", return_value="/safe/path/10_photo.jpg"),
            patch("profile.import_service.activity_media_crud.create_activity_medias"),
        ):
            await service.collect_and_import_activity_components(
                [],
                [],
    ...
    async def test_collect_and_import_activity_components_with_media_no_underscore(self) -> None:
        ...
        with patch("profile.import_service.activity_media_crud.create_activity_medias") as mock_create:
            await service.collect_and_import_activity_components(
                [],
                [],
                [],
    ...
    async def test_collect_and_import_activity_components_with_media_http_exception(self) -> None:
        ...
        with (
            patch(
                "profile.import_service.file_uploads.resolve_storage_path",
                side_effect=HTTPException(status_code=400, detail="unsafe path"),
            ),
            patch("profile.import_service.activity_media_crud.create_activity_medias") as mock_create,
        ):
            await service.collect_and_import_activity_components(
                [],
                [],
...
class TestImportServiceActivitiesDataBatched:
    async def test_collect_and_import_activities_batched_single_batch(self) -> None:
        ...
        with (
            patch(
                "profile.import_service.activities_crud.create_activity",
                new_callable=AsyncMock,
...
class TestImportServiceAddActivityFiles:
    async def test_add_activity_files_from_zip_valid(self) -> None:
        ...
        with (
            patch("profile.import_service.file_uploads.file_validator", mock_validator),
            patch("profile.import_service.file_uploads.save_validated_bytes", new_callable=AsyncMock),
            zipfile.ZipFile(BytesIO(zip_data)) as z,
        ):
    ...
    async def test_add_activity_files_from_zip_no_mapping(self) -> None:
        ...
        with (
            patch("profile.import_service.file_uploads.save_validated_bytes", new_callable=AsyncMock) as mock_save,
            zipfile.ZipFile(BytesIO(zip_data)) as z,
        ):
            await service.add_activity_files_from_zip(
                z,
    ...
    async def test_add_activity_files_from_zip_http_exception(self) -> None:
        ...
        with (
            patch("profile.import_service.file_uploads.file_validator", mock_validator),
            patch(
                "profile.import_service.file_uploads.save_validated_bytes",
                new_callable=AsyncMock,
...
class TestImportServiceHealthEmptyBranches:
    ...
    async def test_collect_and_import_health_targets_no_current(self) -> None:
        ...
        with (
            ...
            patch("profile.import_service.health_targets_crud.edit_health_target"),
            patch("profile.import_service.health_targets_schema.HealthTargetsUpdate", return_value=MagicMock()),
        ):
            await service.collect_and_import_health_weight(
                [{"weight": 75.5}],
                [{"weight": 80.0, "id": 99}],
    ...
    async def test_collect_and_import_health_weight_empty_targets(self) -> None:
        ...
        with patch("profile.import_service.health_weight_crud.create_health_weight"):
