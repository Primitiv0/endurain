...
import activities.activity_streams.constants \
    as activity_streams_constants
...
_STREAM_HIDE_MAP: dict[int, str] = {
    ...
    .STREAM_TYPE_CADENCE: "hide_cadence",
    activity_streams_constants
    .STREAM_TYPE_ELEVATION: "hide_elevation",
    activity_streams_constants
    .STREAM_TYPE_SPEED: "hide_speed",
    activity_streams_constants
    .STREAM_TYPE_PACE: "hide_pace",
    activity_streams_constants
    .STREAM_TYPE_MAP: "hide_map",
}
...
def is_stream_hidden(
    ...
    return bool(
        attr and getattr(activity, attr, False)
    )
...
def filter_visible_streams(
    ...
    return [
        s
        for s in streams
        if not is_stream_hidden(
            activity, s.stream_type
        )
    ]
...
def transform_activity_streams(
    ...
    if (
        ...
        return transform_activity_streams_hr(
            activity_stream, activity, db
        )

    return (
        activity_streams_schema.ActivityStreams
        .model_validate(activity_stream)
    )
...
def transform_activity_streams_hr(
    ...
    if not waypoints or not isinstance(
        waypoints, list
    ):
        return schema

    detail_user = users_crud.get_user_by_id(
        activity.user_id, db
    )
    if not detail_user:
        return schema
