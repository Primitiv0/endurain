...
class TestExportServiceCollectGearData:
    def test_collect_gear_empty_components(self) -> None:
        ...
        with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED, compresslevel=6) as z:
            ...
            with (
                patch("profile.export_service.gear_crud.get_gear_user", return_value=[]),
                patch("profile.export_service.gear_components_crud.get_gear_components_user", return_value=[]),
            ):
...
class TestExportServiceGenerateExportArchiveEdgeCases:
    ...
    def test_bad_zip_file_raises_zip_creation_error(self) -> None:
        ...
        with (
            patch(
                "profile.export_service.zipfile.ZipFile",
