...
import users.users_sessions.router as users_sessions_router
...
def sessions_app(mock_db) -> FastAPI:
    ...
    app.dependency_overrides[auth_security.check_scopes] = lambda: None
...
class TestReadSessionsUser:
    ...
    @patch(
        "users.users_sessions.router.users_session_crud.get_user_sessions"
    )
    @patch("users.users_sessions.router.core_config.settings")
...
class TestDeleteSessionUser:
    ...
    @patch(
        "users.users_sessions.router.users_session_crud.delete_session"
    )
    ...
    def test_delete_nonexistent_session_returns_404(
        ...
        response = sessions_client.delete(
            "/sessions/no-such-session/user/1"
        )
